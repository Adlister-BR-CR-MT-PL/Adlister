...
public class MySQLCategoriesDao implements Categories {
    ...
    public Category getCategoryByName(String cat) {
        PreparedStatement stmt = null;
        try {
            stmt = connection.prepareStatement("SELECT * FROM categories WHERE name LIKE ? LIMIT 1;"); // possible bug here
...
//        seed missing categories into database if not empty
            ...
            for(Category c : dbList){
                for(String name : seedList){
                    if(!seedList.contains(c.getName())){
                        insertIntoCategories(name);
                    }
                }
            }
    ...
    @Override
    public long insertIntoCategories(String name){
        try {
